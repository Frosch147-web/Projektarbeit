...
function spielanzeigeAktualisieren() {
  ...
  if (aktuelleKlasse === SPIELER_KLASSE) {
    ...
    spielanzeige.innerText = `Du bist am Zug, ${playerName}!`;
  } else {
...
function spielBeenden(unentschieden) {
  ...
  // Funktion zum Abrufen der Anzahl der Spiele für einen Spieler
function getGamesPlayed(playerName) {
  const gamesPlayedData = localStorage.getItem('gamesPlayedData');
  if (gamesPlayedData) {
    const parsedData = JSON.parse(gamesPlayedData);
    return parsedData[playerName] || 0;
  }
  return 0;
}

  // Funktion zum Inkrementieren der Anzahl der Spiele für einen Spieler
function incrementGamesPlayed(playerName) {
  const gamesPlayedData = localStorage.getItem('gamesPlayedData');
  let parsedData = gamesPlayedData ? JSON.parse(gamesPlayedData) : {};
  parsedData[playerName] = (parsedData[playerName] || 0) + 1;
  localStorage.setItem('gamesPlayedData', JSON.stringify(parsedData));
}

  // Funktion zum Anzeigen der Anzahl der Spiele für einen Spieler
function displayGamesPlayed() {
  const playerName = document.getElementById('playerName').value;
  const gamesPlayed = getGamesPlayed(playerName);
  console.log(`${playerName} hat ${gamesPlayed} Spiele gespielt.`);
}

  // Funktion, um nach einem Spiel die Anzahl der Spiele zu inkrementieren
function gameEnd() {
  const playerName = document.getElementById('playerName').value;
  incrementGamesPlayed(playerName);
  displayGamesPlayed();
}
